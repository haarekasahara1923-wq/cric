...
export class LeaderboardService implements OnModuleInit {
    ...
    if (redisUrl) {
      try {
        ...
        this.redis.on('error', (err) => {
          console.warn('⚠️ Redis Error (Leaderboard):', err.message);
        });
      } catch (err) {
  ...
  async updateScore(userId: string, points: number) {
    ...
    try {
      ...
      console.warn('Skipping Redis score update:', e.message);
    }
  }
  ...
  async getTopPlayers(period: string = 'season', limit: number = 10) {
    ...
    try {
      ...
      console.warn('Redis read failed (top players):', e.message);
      return [];
    }
  }

  async getUserRank(userId: string, period: string = 'season') {
    if (!this.redis) return { rank: null, score: 0 };
    try {
      const key = period === 'season' ? 'leaderboard:season' : `leaderboard:daily:${new Date().toISOString().split('T')[0]}`;
      ...
      console.warn('Redis read failed (user rank):', e.message);
